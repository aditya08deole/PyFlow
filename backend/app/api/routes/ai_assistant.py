...
async def explain_code(request: dict):
    ...
    try:
        ...
        explanation = f"This code appears to define functions and perform operations. " \
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Explanation service error: {str(e)}"
        )
...
async def fix_errors(request: dict):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error fixing service error: {str(e)}"
        )