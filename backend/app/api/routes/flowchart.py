...
async def generate_flowchart(request: CodeRequest):
    ...
    try:
        ...
        return FlowchartResponse(mermaid=mermaid_code, nodes=nodes)
    except SyntaxError as e:
        raise HTTPException(
            status_code=400, detail=f"Syntax error in Python code: {str(e)}"
        )
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Error generating flowchart: {str(e)}"
        )
