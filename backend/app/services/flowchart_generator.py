...
class FlowchartGenerator:
    ...
    def create_node(self, label: str, node_type: str = "process", line_no: int = None) -> str:
    ...
    def process_statement(self, stmt, current_node: str) -> str:
        ...
        if isinstance(stmt, ast.Assign):
            ...
            decision_node = self.create_node(f"If {condition}?", "decision", stmt.lineno)
            ...
            loop_node = self.create_node(f"For {target} in {iter_expr}", "decision", stmt.lineno)
            ...
            # Function definition
            func_node = self.create_node(f"Function: {stmt.name}", "process", stmt.lineno)
            ...
            generic_node = self.create_node(f"Statement: {stmt_type}", "process", getattr(stmt, 'lineno', None))
