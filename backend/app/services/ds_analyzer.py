...
class DataStructureAnalyzer:
        ...
        for target in node.targets:
            if isinstance(target, ast.Name):
                ...
                if isinstance(node.value, ast.List):
                    ...
                    for elem in node.value.elts:
                        if isinstance(elem, (ast.Constant, ast.Num)):
                            elements.append(
                                elem.n if hasattr(elem, "n") else elem.value
                            )
                ...
                # Dictionary creation
                    ...
                    self._add_operation(
                        "create_hashmap", var_name, line_no, hashmap_info
                    )
        ...
        if isinstance(node.func, ast.Attribute):
            ...
            if isinstance(node.func.value, ast.Name):
                ...
                if method_name in ["append", "insert", "remove", "pop", "extend"]:
                    ...
                    self._add_operation(
                        f"list_{method_name}", obj_name, line_no, operation_info
                    )

                # Stack operations (if object treated as stack)
                elif method_name in ["push"] or (
                    method_name == "append" and self._is_stack_usage(obj_name)
                ):
                    operation_info = {
                        "object": obj_name,
                        "method": "push",
    ...
    def _add_operation(
        self, op_type: str, obj_name: str, line_no: int, details: Dict[str, Any]
    ):
...
class ExecutionTracer:
    ...
    def _simulate_execution(
        ...
        current_ds_state = {"arrays": [], "stacks": [], "variables": {}}
    ...
    def _update_ds_state(
        self, line: str, current_state: Dict[str, Any]
    ) -> Dict[str, Any]:
