...
const DataStructureViewer: React.FC<DataStructureViewerProps> = ({ dsState }) => {
  const renderArray = (arr: ArrayData) => (
    <div className="bg-white p-4 rounded border shadow-sm">
      ...
      {/* Operation info */}
      {arr.operation && (
        <div className="mt-3 text-sm text-blue-600 bg-blue-50 p-2 rounded">{arr.operation}</div>
      )}
    </div>
  );
      ...
      <div className="flex items-center space-x-2 overflow-x-auto pb-2">
        {list.nodes.map((node: LinkedListNode, i: number) => (
          <React.Fragment key={i}>
            ...
            {i < list.nodes.length - 1 && <div className="flex-shrink-0 text-gray-400">→</div>}
          </React.Fragment>
        ))}
        {list.nodes.length === 0 && <div className="text-gray-500 text-sm">Empty list</div>}
      </div>

      {list.operation && (
        <div className="mt-3 text-sm text-blue-600 bg-blue-50 p-2 rounded">{list.operation}</div>
      )}
    </div>
  );
  ...
  const renderStack = (stack: Stack) => (
    <div className="bg-white p-4 rounded border shadow-sm">
      ...
      <div className="mt-2 text-xs text-gray-500 text-center">↑ Top of stack</div>

      {stack.operation && (
        <div className="mt-3 text-sm text-blue-600 bg-blue-50 p-2 rounded">{stack.operation}</div>
      )}
    </div>
  );
  ...
  const renderBinaryTree = (tree: Tree) => (
    <div className="bg-white p-4 rounded border shadow-sm">
      ...
      <div className="min-h-32 flex items-center justify-center">
        {tree.root ? (
          <div className="text-center">
            ...
            <div className="text-sm text-gray-600">Tree structure visualization</div>
            <div className="text-xs text-gray-500 mt-1">Root: {tree.root.value}</div>
          </div>
        ) : (
      ...
      {tree.operation && (
        <div className="mt-3 text-sm text-blue-600 bg-blue-50 p-2 rounded">{tree.operation}</div>
      )}
    </div>
  );
  ...
  if (!dsState) {
    return (
      <div className="p-8 text-center text-gray-500">
        ...
        <p className="text-sm">
          Create arrays, lists, stacks, or other data structures in your code to see them visualized
          here!
        </p>
  ...
  return (
    <div className="space-y-6 p-4">
      ...
      {dsState.arrays?.map((arr: ArrayData, idx: number) => (
        <div key={`array-${idx}`}>{renderArray(arr)}</div>
      ))}

      {/* Linked Lists */}
      {dsState.linkedLists?.map((list: LinkedList, idx: number) => (
        <div key={`list-${idx}`}>{renderLinkedList(list)}</div>
      ))}

      {/* Stacks */}
      {dsState.stacks?.map((stack: Stack, idx: number) => (
        <div key={`stack-${idx}`}>{renderStack(stack)}</div>
      ))}

      {/* Binary Trees */}
      {dsState.trees?.map((tree: Tree, idx: number) => (
        <div key={`tree-${idx}`}>{renderBinaryTree(tree)}</div>
      ))}
        ...
        <div className="bg-gradient-to-r from-purple-50 to-blue-50 p-4 rounded border">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            {dsState.metrics.swaps && (
              <div className="text-center">
                <div className="text-lg font-semibold text-green-600">{dsState.metrics.swaps}</div>
